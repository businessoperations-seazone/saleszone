...
def test_patch_registration_requires_auth(client):
    """Regressão: PATCH /api/admin/registrations/<id> precisa do @require_admin
    aplicado (via blueprint before_request). Sem Authorization header → 401."""
    resp = client.patch(
        "/api/admin/registrations/r1",
        json={"observacoes": "x"},
    )
...
def test_patch_registration_rejects_opportunity_marked_at_from_client(client):
    """Cliente não pode forçar opportunity_marked_at via payload — o backend
    deriva esse campo ao ver is_opportunity=True pela primeira vez."""
    reg_before = {"id": "r1", "session_id": "s1", "is_opportunity": None}
    reg_after = {**reg_before, "observacoes": "x"}
    with patch("routes.admin.require_admin", return_value=MOCK_ADMIN), \
         patch("routes.admin.db.select", return_value=[reg_before]), \
         patch("routes.admin.db.update", return_value=[reg_after]) as mock_update, \
         patch("routes.admin.webinar_fup.handle_patch_update"):
        resp = client.patch(
            "/api/admin/registrations/r1",
            json={
                "observacoes": "x",
                "opportunity_marked_at": "1999-01-01T00:00:00Z",  # tentativa de forjar
            },
            headers=_admin_headers(),
        )
    assert resp.status_code == 200
    # payload enviado ao supabase não deve conter opportunity_marked_at
    payload_sent = mock_update.call_args[1].get("data") or mock_update.call_args[0][2]
    assert "opportunity_marked_at" not in payload_sent

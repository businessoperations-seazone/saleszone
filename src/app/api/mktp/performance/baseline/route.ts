...
// TODO: MKTP closer hire dates — fill in when team structure is discovered
  ...
  try {
    ...
    for (const name of V_COLS) {
      ...
      for (const d of deals) {
        ...
        if (d.max_stage_order >= 8) m.opp++; // MKTP OPP = Reunião Realizada (order 8)
    ...
    console.error("MKTP Baseline error:", error);

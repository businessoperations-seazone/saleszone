  ...
  for (const stageId of PIPELINE_STAGES) {
    ...
    while (true) {
      ...
      for (const deal of res.data) {
        ...
        rows.push(dealToRow(deal, 13, true));
      }
...
// Populates reserva_entered_at (stage 305) and contrato_entered_at (stage 271)
// com LAST timestamp por deal no MKTP pipeline (37).
async function syncBackfillStageEntries(nektApiKey: string, supabase: any) {
  console.log(`syncBackfillStageEntries MKTP: querying Nekt stage entries into 305/271...`);
  const sql = `
    SELECT CAST(item_id AS varchar) AS deal_id,
           new_value AS stage,
           MAX(log_time) AS entered_at
    FROM nekt_trusted.pipedrive_v2_consolidated_deal_flow
    WHERE field_key = 'stage_id' AND new_value IN ('305', '271')
  ...
  for (const r of rows) {
    ...
    if (r.stage === "305") entry.reserva = ts;
    else if (r.stage === "271") entry.contrato = ts;
  }
  ...
  for (let i = 0; i < updates.length; i += CONCURRENCY) {
    ...
    const results = await Promise.all(chunk.map(async (u) => {
      ...
      const { error } = await supabase.from("mktp_deals").update(patch).eq("deal_id", u.deal_id);
      return { ok: !error };
    }));
